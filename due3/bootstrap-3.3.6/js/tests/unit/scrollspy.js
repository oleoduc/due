$(function () {
  ...
  QUnit.test('should correctly select middle navigation option when large offset is used', function (assert) {
    ...
    $scrollspy.on('scroll.bs.scrollspy', function () {
      assert.ok(!$section.find('#one-link').parent().hasClass('active'), '"active" class removed from first list')
      assert.ok($section.find('#two-link').parent().hasClass('active'), '"active" class on middle list')
      assert.ok(!$section.find('#three-link').parent().hasClass('active'), '"active" class not on last list')
      done()
    })
  ...
  QUnit.test('should clear selection if above the first list', function (assert) {
    assert.expect(3)
    var done = assert.async()
    ...
    $scrollspy
      ...
      .one('scroll.bs.scrollspy', function () {
        ...
        assert.strictEqual($('.active').has('#two-link').length, 1, '"active" class on second list')
  ...
  QUnit.test('should correctly select navigation element on backward scrolling when each target list height is 100%', function (assert) {
    assert.expect(5)
    var navbarHtml =
        '<nav class="navbar">'
      + '<ul class="nav">'
